...
class commissiongroupController extends BaseController {
    ...
    public function querycommissiongroup(){
        ...
        if ($mode == 'collection') {
            ...
            $commission = commissiongroup::select('*');


           return Datatables::of($commission)   ->addColumn('link', function ($p) {
               //  if(Auth::user()->can('edit_product'))
               if(Auth::user()->can('edit_commissiongroup'))
                   return '<span onclick="editcommissiongroup(\''.$p->id.'\')" class="btn btn-xs default"><i class="fa fa-search"></i> 修改</span>';
               else
                   return '';
           })->make(true);
        }
